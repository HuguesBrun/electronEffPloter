...
TFile *myFile = new TFile("theSFplots.root");
...
drawMyPlot(TString nameCut, TString theText, TString title, float yMin, float yMax){
    ...
    cout << nameCut+"_All" << endl;
	
	TGraphAsymmErrors *theGraph4 = (TGraphAsymmErrors*) myFile->Get(nameCut+"_All");
    ...
    TGraphAsymmErrors *theGraph4B = (TGraphAsymmErrors*) myFile->Get(nameCut+"_RunAB");
    ...
    TGraphAsymmErrors *theGraph4C = (TGraphAsymmErrors*) myFile->Get(nameCut+"_RunC");
    ...
    TGraphAsymmErrors *theGraph4D = (TGraphAsymmErrors*) myFile->Get(nameCut+"_RunD");
 ...
 //   line->Draw("same");
	...
	c0->Print("compSF/plot_"+nameCut+"MC.png");
...
doADraw5bins(TString name, TString title){
    cout << "on va plotter le plot " << name << endl;
    drawMyPlot(name, "|SC #eta| < 0.8",title,0.5,1.1);
    name.ReplaceAll("bin0","bin1");
	drawMyPlot(name, "0.8 <|SC #eta| < 1.4442",title,0.5,1.1);
    name.ReplaceAll("bin1","bin2");
	drawMyPlot(name, "1.4442 <|SC #eta| < 1.556",title,0.5,1.1);
    name.ReplaceAll("bin2","bin3");
	drawMyPlot(name, "1.556 <|SC #eta| < 2.0",title,0.5,1.1);
    name.ReplaceAll("bin3","bin4");
	drawMyPlot(name, "2.0 <|SC #eta| < 2.5",title,0.5,1.1);
...
doThePlotsMC_SF(){
    ...
    doADraw5bins("SF_passFO_BDT_ISO_pt_pt_PLOT_absSCeta_bin0_&_isSameSign_fail","Hww selection");
    doADraw5bins("SF_passFO_ptFO_pt_PLOT_absSCeta_bin0_&_isSameSign_fail","Preselection");
    doADraw5bins("SF_passFO_BDT_ISO_ptNM1ID_pt_PLOT_absSCeta_bin0_&_isSameSign_fail_&_passFO_ISO_pass","N-1 ID");
    doADraw5bins("SF_passFO_BDT_ISO_ptNM1ISO_pt_PLOT_absSCeta_bin0_&_isSameSign_fail_&_passFO_BDT_pass","N-1 ISO");
